    ...
    <nav className={showNav ? 'menuContainer active' : 'menuContainer'}>
      <ul>
        {SidebarData.map((item, index) =>{
          return (
            <li key={index} className={showNav ? `${item.cName} active` : `${item.cName}`}>
              <Link to={item.path} className={showNav ? `itemList active` : `itemList`}>
                <img src={`${item.icon}`} alt={item.cName} />
                <span className={showNav ? 'menuTitle active' : 'menuTitle'}>{item.title}</span>
              </Link>
            </li>
          )
        })}
